 ...
 */
public class PropertiesValidatorNew extends BaseJsonValidator {
    public static final String PROPERTY = "properties";
    private static final Logger logger = LoggerFactory.getLogger(PropertiesValidatorNew.class);
    private final Map<String, JsonSchema> schemas = new LinkedHashMap<>();
    
    private Boolean hasUnevaluatedPropertiesValidator;
    
    public PropertiesValidatorNew(SchemaLocation schemaLocation, JsonNodePath evaluationPath, JsonNode schemaNode, JsonSchema parentSchema, ValidationContext validationContext) {
    ...
    protected Set<ValidationMessage> validate(ExecutionContext executionContext, JsonNode node, JsonNode rootNode,
        ...
        for (Entry<String, JsonSchema> entry : this.schemas.entrySet()) {
            JsonNode propertyNode = this.validationContext.getConfig().getNodeAccessor().get(node, entry.getKey());
    ...
    public Set<ValidationMessage> walk(ExecutionContext executionContext, JsonNode node, JsonNode rootNode, JsonNodePath instanceLocation, boolean shouldValidateSchema) {
        ...
        if (this.validationContext.getConfig().getApplyDefaultsStrategy().shouldApplyPropertyDefaults() && null != node
                && this.validationContext.getConfig().getNodeAccessor().isObject(node)) {
            applyPropertyDefaults(node);
        }
        if (shouldValidateSchema) {
            validationMessages.union(validate(executionContext, node == null ? this.validationContext.getConfig().getNodeAccessor().missingNode() : node, rootNode,
    ...
    private void applyPropertyDefaults(JsonNode node) {
        for (Map.Entry<String, JsonSchema> entry : this.schemas.entrySet()) {
            JsonNode propertyNode = this.validationContext.getConfig().getNodeAccessor().get(node, entry.getKey());

            JsonNode defaultNode = getDefaultNode(entry.getValue(), this.validationContext.getConfig().getNodeAccessor());
            if (defaultNode == null) {
                continue;
            }
            ...
            if (applyDefault) {
            	this.validationContext.getConfig().getNodeAccessor().set(node, entry.getKey(), defaultNode);
            }
        }
    }

    private static JsonNode getDefaultNode(JsonSchema schema, NodeAccessor<JsonNode> nodeAccessor) {
        JsonNode result = nodeAccessor.get(schema.getSchemaNode(), "default");
        if (result == null) {
            JsonSchemaRef schemaRef = JsonSchemaRefs.from(schema);
            if (schemaRef != null) {
                result = getDefaultNode(schemaRef.getSchema(), nodeAccessor);
            }
        }
        return result;
    }
