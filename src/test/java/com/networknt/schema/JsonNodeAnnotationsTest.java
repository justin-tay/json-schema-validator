...
class JsonNodeAnnotationsTest {
    ...
    void filter() {
        ...
        JsonNodePath instance = instanceRoot.resolve("foo");

        JsonNodePath evaluationRoot = new JsonNodePath(PathType.JSON_POINTER).resolve("properties").resolve("foo");
        ...
        annotations.put(JsonNodeAnnotation.builder().keyword("title").instanceLocation(instance)
                .evaluationPath(evaluationRoot.resolve("$ref").resolve("title")).value("Title in reference target")
                .build());
        annotations.put(JsonNodeAnnotation.builder().keyword("title").instanceLocation(instance)
                .evaluationPath(evaluationRoot.resolve("title")).value("Title adjacent to reference").build());
        annotations.put(JsonNodeAnnotation.builder().keyword("description").instanceLocation(instance)
                .evaluationPath(evaluationRoot.resolve("$ref").resolve("description")).value("Even more text").build());
        annotations.put(JsonNodeAnnotation.builder().keyword("description").instanceLocation(instance)
                .evaluationPath(evaluationRoot.resolve("description")).value("Lots of text").build());
        // 1 instance
